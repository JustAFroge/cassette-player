...
(function (window, $, undefined) {
  ...
  $.Cassette.prototype = {
    ...
    /* responsive rim scale for tape wheels */
    _computeScale: function () {
      var BASE = 586; // original tape width
      var tapeW = (this.$theTape && this.$theTape.width) ? this.$theTape.width() : BASE;
      this._scale = tapeW / BASE;
      this._rimMax = 70 * this._scale; // was fixed 70px before
    },
        ...
        for (var i = 0, len = _self.options.songs.length; i < len; ++i) {
          ...
          $.when(song.loadMetadata()).done(function (song) {
            ...
            if (cnt === len) {
              ...
              _self.side1 = new $.Side('side1', playlistSide1, 'start');   // ← changed
              _self.side2 = new $.Side('side2', playlistSide2, 'end');     // ← changed
              dfd.resolve();
            }
          });
        }
    ...
    _createPlayer: function () {
      ...
      // keep wheels’ rim size responsive to the tape width
      this._computeScale();
      var _self = this;
      $(window).on('resize.cassette', function () { _self._computeScale(); });

      // seed initial wheel values using responsive rim
      this.$wheelLeft.data('wheel', this._rimMax);
      this.$wheelRight.data('wheel', 0);
    },
    ...
    _loadEvents: function () {
      ...
      this.$cSwitch.on('mousedown', function () {
        _self._setButtonActive($(this));
        _self._switchSides();
      });

      this.$cPlay.on('mousedown', function () {
        _self._setButtonActive($(this));
        _self._play();
      });

      this.$cStop.on('mousedown', function () {
        _self._setButtonActive($(this));
        _self._stop();
      });

      this.$cForward.on('mousedown', function () {
        _self._setButtonActive($(this));
        _self._forward();
      });

      this.$cRewind.on('mousedown', function () {
        _self._setButtonActive($(this));
        _self._rewind();
      });
      ...
      this.$audioEl.on('loadedmetadata', function () {});
    ...
    _updateAction: function (action) { this.lastaction = action; },
    ...
    _switchSides: function () {
      ...
      if (this.currentSide === 1) {
        ...
        setTimeout(function () {
          ...
          _self.cntTime = _self._getPosTime(); // update wheels
        }, 200);
    ...
    _changeVolume: function (ratio) { this.audio.volume = ratio; },
    ...
    _play: function () {
      ...
      $.when(this.sound.play('click')).done(function () {
        ...
        if (data) {
          ...
          _self.$audioEl.on('canplay', function () {
            $(this).off('canplay');
    ...
    _rewind: function () {
      ...
      if (this._getSide().current.getPositionStatus() === 'start') return false;
    ...
    _forward: function () {
      ...
      if (this._getSide().current.getPositionStatus() === 'end') return false;
    ...
    _clear: function () { this.$audioEl.children('source').remove(); },
    ...
    // given a point in time for the current side, returns the respective song and time
    ...
    /* use responsive rim instead of hardcoded 70 */
    _getWheelValues: function (x) {
      var T = this._getSide().current.getDuration();
      var R = this._rimMax || 70; // fallback if not computed yet
      return {
        left: (this.currentSide === 1) ? (-R / T) * x + R : (R / T) * x,
        right: (this.currentSide === 1) ? (R / T) * x : (-R / T) * x + R
      };
    },
    ...
    _getPosTime: function () {
      ...
      var R = this._rimMax || 70;

      if (wleft === undefined) wleft = R;
      if (wright === undefined) wright = 0;

      var T = this._getSide().current.getDuration();
      return this.currentSide === 2 ? (T * wleft) / R : (T * wright) / R;
    },
    ...
    _setWheelAnimation: function (speed, mode) {
      ...
      switch (this.currentSide) {
        case 1:
          anim = (mode === 'rewind') ? 'rotateRight' : 'rotateLeft';
          break;
        case 2:
          anim = (mode === 'rewind') ? 'rotateLeft' : 'rotateRight';
          break;
      }
    ...
    _resetElapsed: function () { this.elapsed = 0.0; }
  };

  // Cassette side obj
  $.Side = function (id, playlist, status) {
    this.id = id;             // side's name / id
    this.status = status;     // "start", "middle" or "end"
    this.playlist = playlist.sort(function (a, b) {
      var aid = a.id, bid = b.id;
      return ((aid < bid) ? -1 : ((aid > bid) ? 1 : 0));
    });
    this._setDuration();      // set playlist duration
    this.playlistCount = playlist.length; // total number of songs
  };
  ...
  $.Song.prototype = {
    ...
    getSource: function (type) { return this.sources[type]; },
    ...
    loadMetadata: function () {
      ...
      return $.Deferred(function (dfd) {
        ...
        $tmpAudio.on('loadedmetadata', function () {
  ...
  $.Sound.prototype = {
    ...
    getSource: function (type) { return 'sounds/' + this.action + '.' + type; },
    ...
    play: function (action, loop) {
      ...
      return $.Deferred(function (dfd) {
        ...
        _self.$audio.on('canplay', function () {
